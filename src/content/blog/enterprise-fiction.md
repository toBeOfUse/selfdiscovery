...
Another example that I like to give is the role of the director of a movie. The director will probably have a lot of control over the script and the performances, but the details of things like costuming, lighting, and even shot composition end up getting left to others, who will take that high-level vision on and fulfill it. Peter Jackson [gave a speech](https://www.youtube.com/shorts/FqT5jmTLPOc) to the design crew of the Lord of the Rings movies where he said that he needed them to think about their work as if they were depicting real events from history; as if the people that they were portraying had been real, and that everything needed to be accurate to that time period; and that they were shooting on the locations where these events actually happened. To ask people to forget that they're working on a fantasy movie at all is to invoke an extremely high level of abstraction. What could go wrong?
...
Unfortunately, I do believe this: there is no "correct" level of abstraction to look at things with. Emails are both the electrons and the election fundraising pleas. Both the vision and the execution for the software needs to be solid.

It's common for people, at least in tech, to try to start businesses with no real high-level idea of how things will work out; they just want to focus on the implementation level, i.e. building something. I've been to startup incubator pitch sessions where people stumble when asked about their revenue model, and just want to talk about the cool platform they're building to let people plan events or set up playdates for their pets or whatever. I've worked with people who just want to create what feels right to them, and end up implementing stuff like a bespoke notification system that's interesting and makes sense and, at a high level, adds nothing to the user experience over sending a basic email.

It would be easy for the app deployment platform I'm working on to turn into something like Dokploy or Coolify, that tries to clone the functionality of existing platforms like Netlify or Vercel. Maybe it still will. Open source software, in general, is not great about having its own executive direction; that's why the most popular stuff in open source software has always been things like clones of Photoshop and Microsoft Word. The high-level concerns are already taken care of, on a project like that; someone just needs to tackle the implementation, and there are plenty of people excited to do that.

And the high-level vision and the mid-level implementation have to match each other. I think that it's an anti-pattern to have business planning and technical implementation be handled in different silos. Even if your company has a great twist on the concept "Uber for dog-walking," in my experience, it's easy for the implementation level to miss that and just implement whatever [Swifto](https://swifto.com/) was doing already.
...
The important thing is to understand things in terms of _roles_; both the executive role and the implementation role are important.
